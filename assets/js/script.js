...
let specialOffers__wrapper_akcii = new Swiper(".specialOffers .specialOffers__wrapper-akcii", {
    slidesPerView: 3,
    spaceBetween: 20,
    rewind: true,
    pagination: {
...
document.addEventListener('DOMContentLoaded', function () {
    const dropdowns = document.querySelectorAll('.header__menu .dropdown-toggle');
...
document.querySelectorAll('.header__menu.navbar .nav-link').forEach(link => {
    link.addEventListener('click', event => {
        // event.preventDefault(); // Блокировка стандартного поведения
...
document.addEventListener('click', (e) => {
    const navLinkEl = e.target.closest('.offcanvas-body .nav-link');
    if (!navLinkEl) {
        return;
    }
